...
import sabre
from agent.bandwidth_source import SabreBandwidthSource
...
class Ours(sabre.Abr):
    def __init__(self, config):
        ...
        self.bandwidth_source = SabreBandwidthSource(self.session)

    def get_quality_delay(self, segment_index):
        bitrates_kbps = self.session.manifest.bitrates
        bandwidth_kbps = self.bandwidth_source.get_bandwidth_estimate()
